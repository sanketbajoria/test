...
L.Handler.Draw = L.Handler.extend({
	...
	_createMarker: function (latlng, dir, index) {
		var marker = new L.Marker(latlng, {
			icon: new L.DivIcon({
				iconSize: new L.Point(8, 8),
				className: 'leaflet-div-icon leaflet-editing-icon leaflet-marker-resize-' + dir
			}),
	...
	_updateMarker: function(marker,latlng){
		if(latlng.lat)
	...
	_updateMarkers: function(shape){
		var latlngs = this._getMarkerLatlngs(shape);
		for(var i=0;i<shape._markers.length;i++){
			this._updateMarker(shape._markers[i],latlngs[i]);
		}
	},
	_getMiddleLatLng: function(latlng1,latlng2){
		return new L.LatLng((latlng1.lat + latlng2.lat)/2,(latlng1.lng + latlng2.lng)/2);
	},
	_editMode : function(e){
		this._createMarkerGroup(e.target);
		if(e.target.editMode){
			this._map.removeLayer(e.target._markerGroup);
		}
		else{
			this._map.addLayer(e.target._markerGroup);
		}
		e.target.editMode = !e.target.editMode;
		return false;
	},
	_initShape: function(shape){
		shape.editMode = false;
		shape.on("click",this._editMode,this);
		shape.on("dblclick",function(e){L.DomEvent.stop(e.originalEvent)});
	},
});
...
L.Polyline.Draw = L.Handler.Draw.extend({
	...
	_onClick: function (e) {
		...
		this._markers.push(this._createMarkers(latlng));
	...
	_createMarkers: function (latlng) {
		var marker = new L.Marker(latlng, {
	...
	_getMarkerLatlngs : function(shape){
		var ret = shape.getLatLngs();
		return ret;
	},
	
	
	
	_splitMarker: function(shape,latlng,dir,index){
		 var that = this;
		 var marker = this._createMarker(latlng,dir,index).on("dragend",function(){
			 if(!this.dragged){
				 var index = this.index;
				 var latlngs = that._getMarkerLatlngs(shape);
				 var len = latlngs.length;
				 var pin = (index-1+len)%len,nin = (index+1+len)%len;
				 var next = that._getMiddleLatLng(latlngs[index],latlngs[nin]), pre = that._getMiddleLatLng(latlngs[index],latlngs[pin]);
				 shape.spliceLatLngs(index,0,pre);
				 shape.spliceLatLngs(index+2,0,next);
				 var nextM = that._splitMarker(shape,next,"dfd",index+2);var preM = that._splitMarker(shape,pre,"dfd",index);
				 shape._markers.splice(index,0,preM);
				 shape._markers.splice(index+2,0,nextM);
				 for(var i=0;i<shape._markers.length;i++){
					 shape._markers[i].index = i;
				 }
				 shape._markerGroup.addLayer(preM);
				 shape._markerGroup.addLayer(nextM);
				 this.dragged = true;
				 }
		}).on("drag",function(){
			var latlngs = that._getMarkerLatlngs(shape);
			latlngs[this.index] = this.getLatLng();
			shape.setLatLngs(latlngs);
			that._updateMarkers(shape);
		});
		 marker.dragged = false;
		 return marker
	},
	
	_createMarkerGroup: function(shape){
		var that = this;
		if(!shape._markerGroup){
			var markerLatlngs = this._getMarkerLatlngs(shape);
			shape._markerGroup = new L.LayerGroup();
			shape._markers = [];
			for(var i=0;i<markerLatlngs.length;i++){
				var that = this;
				shape._markers[i] = that._splitMarker(shape,markerLatlngs[i],'ckj',i);
				shape._markerGroup.addLayer(shape._markers[i]);
			}
		}
	}
});
...
L.Circle.Draw = L.SimpleShape.Draw.extend({
	...
	_getMarkerLatlngs : function(circ){
		var b = circ.getBounds();
		return [b.getCenter(),b.getNorthWest(),b.getNorthEast(),b.getSouthEast(),b.getSouthWest()];
	},
	
	_createMarkerGroup: function(circ){
		var that = this;
		if(!circ._markerGroup){
			var markerLatlngs = this._getMarkerLatlngs(circ);
			circ._markerGroup = new L.LayerGroup();
			circ._markers = [];
			for(var i=0;i<markerLatlngs.length;i++){
				var that = this;
				circ._markers[i] = this._createMarker(markerLatlngs[i],this._dir[i],i).on("drag",function(){
					var latlngs = null;
					if(this.index == 0){
						circ.setLatLng(this.getLatLng());
					}
					else{
						circ.setRadius(circ.getLatLng().distanceTo(this.getLatLng()));
					}
					that._updateMarkers(circ);
					
				});
				circ._markerGroup.addLayer(circ._markers[i]);
			}
		}
	}
});
...
L.Rectangle.Draw = L.SimpleShape.Draw.extend({
	...
	_dir: ['nw','n','ne','e','se','s','sw','w','c'],
	
	_getMarkerLatlngs : function(rect){
		var markerLatlngs = [];
		for(var i=1; i<rect._latlngs.length;i++){
			markerLatlngs.push(rect._latlngs[i]);
			markerLatlngs.push(this._getMiddleLatLng(rect._latlngs[i],rect._latlngs[(i%4)+1]));
		}
		markerLatlngs.push(this._getMiddleLatLng(rect._latlngs[1],rect._latlngs[3]));
		return markerLatlngs;
	},
	
	_createMarkerGroup: function(rect){
		var that = this;
		if(!rect._markerGroup){
			rect._markerGroup = new L.LayerGroup();
			rect._markers = [];
			var markerLatlngs = this._getMarkerLatlngs(rect);
			for(var i=0;i<markerLatlngs.length;i++){
				rect._markers[i] = this._createMarker(markerLatlngs[i],this._dir[i],i).on('dragstart',function(){
					that._startBound = rect.getBounds();
					that._markerStartLatLng = this.getLatLng();
				}).on('drag',function(e){
					var lat = this.getLatLng().lat,lng = this.getLatLng().lng;
					var ne = rect.getBounds()._northEast, sw = rect.getBounds()._southWest, c = rect.getBounds().getCenter();
					if(this.index % 4 !=3){
						if(this.index==8){
							var diff = lat - c.lat;
							ne.lat +=  diff;
					...
					rect.setBounds(new L.LatLngBounds(ne, sw));
					that._updateMarkers(rect);
				});
				rect._markerGroup.addLayer(rect._markers[i]);
			}
		}
