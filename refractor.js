...
(function( $ ) {
    ...
    var actionMap = {}
     ...
     BootstrapEditorFrame.prototype = {
			...
			execCmd			 : function(cmd,val){
								try {
				                    this.frameDoc.execCommand(cmd, false, val);
				                   } catch (e) {
				                     console.error(e);
				                  }
								},
                   ...
                   return true;
                        ...
                        if((e.which>=65 && e.which<=90)||(e.which>=48 && e.which<=57) || (e.which==188 || e.which==190) || (e.which>=112 && e.which<=123))
                        {
                            ...
                            if(e.altKey || e.ctrlKey || e.shiftKey || (e.which>=112 && e.which<=123))   // ctrl -- 0x0100 alt -- 0x0200 shift 0x0400
                            {
                                ...
                                editor.executeAction.apply(editor,[hotkeyMap[hotkey]]);
   ...
   BootstrapEditorToolbar.prototype = {
		...
		init			: function(e){
                            ...
                            this.ui = $(htmlElem).on("click","li[trigger!='hover']:not(.divider,.divider-vertical,.dropdown)",function(e){
	                            ...
	                            e.executeAction(actionKey,$(this),e);
   ...
   var utility = {
		 ...
		 getBootstrapModal 		: function(elem,editor,context)
						            {
   ...
   utility.action = {
		   delTable 		: function(){
			               var tableProp = utility.getTableProp(this);
			...
			delRow 			: function(){
			               ...
			               if(tableProp.rowIndex>=0 && tableProp.rowIndex<tableProp.rowLen){
			                   ...
			                   this.frame.selectNode(sibling[tableProp.rowIndex<tableProp.rowLen-1?tableProp.rowIndex:tableProp.rowIndex-1].cells[tableProp.colIndex>=0?tableProp.colIndex:0],true);
			               }
			            },
			delCol 			: function(){
			               var tableProp = utility.getTableProp(this);
			               ...
			               if(tableProp.colIndex>=0 && tableProp.colIndex<tableProp.colLen){
			                  ...
			                  this.frame.selectNode(sibling[tableProp.colIndex<tableProp.colLen-1?tableProp.colIndex:tableProp.colIndex-1],true);
			               }
			            },
			insCol 			: function(key){
			               var right = actionMap[key].value;
			               var tableProp = table.getTableProp(this);
			...
			insTable 		: function(key,elem,e){
				                ...
				                tableModal = utility.getBootstrapModal(elem,that,{body:body,button:button, callback:function(){
				                                   ...
				                                   tableStr += "</table>";
				                                   that.frame.insertNode(tableStr);
				                                }, initialize: function(){
			...
			changeColor 		: function(key,elem,e){
				               ...
				               ,   cmd = actionMap[key].command || key
				               ,   obj = $(elem.find("a:first")[0])
				               ,   tip = obj.data('popover') && obj.data('popover').tip()
				               ,   val = colorPicker.normalizeColor(queryCommandValue(cmd)) || (fore?"#000":"#fff")
				               ,   that = this;
				               if(!tip){
					           tip = utility.colorPicker.createColorPicker(obj,elem.attr("title"),"right","manual",function(color){
										            tip.hide();
										            that.frame.focus();
										            that.executeAction(key,elem,e,color);
										            });
			...
			viewHtml 		: function(key,elem,e){
							...
							if(!htmlModal){
								var body = '<form><fieldset><label>HTML Source Editor</label><textarea id="htmlSource" rows="15" style="width:100%"></textarea></fieldset></form>'
									htmlModal = utility.getBootstrapModal(elem,this,{body:body,button:'Update',callback:function (){
		                                   ...
		                                   that.frame.frameDoc.body.innerHTML = content;
								}});
							}
			                htmlSource = $('#htmlSource',htmlModal);
			                setTimeout(function(){
			                                    htmlSource.val(that.frame.frameDoc.body.innerHTML);
			                                    htmlModal.modal('show');
			                                    },200);
			            },
			insImg 			: function(key,elem,e){
							var imageModal = elem.data("modal")
							  , imageUrl = null
							  , that = this;
							if(!imageModal){
			                var body = '<form><fieldset><label>Enter Image URL</label><input type="text" id="imageUrl" placeholder="Image URL"></fieldset></form>';
			                imageModal = utility.getBootstrapModal(elem,this,{body:body,button:'Insert',callback:function (){
			                                   imageUrl = imageUrl || $('#imageUrl',imageModal);
			                                   if($.trim(imageUrl.val()) === "") return;
			                                   that.executeAction(key,elem,e,$.trim(imageUrl.val()));
			                }});
			                }
			...
			createLink 		: function(key,elem,e){
							...
							if(!linkModal){
								...
								linkModal = utility.getBootstrapModal(elem,this,{body:body,button:'Create', callback:function(){
	                                   var valid = true;
	                                   address = address || $('#address',linkModal);
	                                   text = text || $('#text',linkModal);
	                                   if($.trim(address.val()) === "" || $.trim(text.val()) === "") return;
	                                   var textNode = that.frame.frameDoc.createTextNode($.trim(text.val()))
	                                   that.frame.replaceText(textNode);
	                                   that.executeAction(key,elem,e,$.trim(address.val()));
	                                }});
							}
			                address = $('#address',linkModal).val("http://");
			                text = $('#text',linkModal).val(that.frame.getText());
    ...
    checkFocusBlurState : function(instant){
	 ...
	 executeAction 		: function (key,elem,e,val) {
				            ...
				            if (!val && action.execCmd) {
				            	...
				            	this.frame.execCmd((action.command||key), (val||action.value))
				            }
				        },	
